    ...
    socket.on("message", async (raw) => {
      ...
      try {
        ...
        if (input.event === "chat.resume") {
          ...
          return send(socket, "chat.completed", { session_id:request.sessionId,message_id:answer.id,response:answer.content,sources:answer.sources || [],confidence:answer.metadata?.confidence || "grounded",answer_outcome:answer.metadata?.answerOutcome || null,handoff:answer.metadata?.handoff || null,request_id:input.request_id,resumed:true }, ++sequence);
        }
        const requestId = input.client_request_id || input.request_id;
        const controller = new AbortController();
        if (controllers.has(requestId)) throw Object.assign(new Error("Request is already active."), {status:409,code:"REQUEST_IN_PROGRESS"});
        ...
        if (controllers.get(input.client_request_id || input.request_id) === ownedController) controllers.delete(input.client_request_id || input.request_id);
        send(socket, "chat.error", { request_id:input.client_request_id || input.request_id,error:"Chat could not complete.",code:error.code || "SERVICE_ERROR",status:error.status || 500 });
      }
    });
