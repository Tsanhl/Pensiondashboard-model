...
const originals = resolve(root,"originals","topical-secondary-legislation");
const index = resolve(root,"index","topical-secondary-legislation");
await Promise.all([mkdir(originals,{ recursive:true }),mkdir(index,{ recursive:true })]);
const allSources = JSON.parse(await readFile(resolve(root,"topical-secondary-legislation-sources.json"),"utf8"));
...
if (requestedIds.size && sources.length !== requestedIds.size) throw new Error("One or more MATERIAL_IDS did not match topical-secondary-legislation-sources.json");
...
async function download(url,destination) {
  ...
  const partial = `${destination}.partial`; await writeFile(partial,buffer); await rename(partial,destination);
...
for (const source of sources) {
  ...
  let original;
  try {
    const downloadedPdf = await download(pdfUrl,pdfPath);
    original = { path:pdfPath,format:"pdf",url:pdfUrl,bytes:downloadedPdf.bytes,sha256:downloadedPdf.sha256 };
  } catch (error) {
    const madeUrl = `${base}/made/data.xml`;
    const made = await download(madeUrl,madeXmlPath);
    original = { path:madeXmlPath,format:"xml",url:madeUrl,bytes:made.bytes,sha256:made.sha256,pdf_unavailable:error.message };
  }
  ...
  if (text.toString("utf8").trim().length < 200 && original.format === "pdf") {
    ...
    if (extracted.text.trim().length < 200) throw new Error(`Could not extract usable text for ${source.title}`);
    await writeFile(textPath,`${extracted.text.trim()}\n`);
    text = await readFile(textPath); textExtraction = "as_made_pdf_fallback";
  }
  const xmlText = await readFile(xmlPath,"utf8");
  const madeDate = xmlText.match(/<ukm:Made\s+Date="([0-9-]+)"/)?.[1] || `${source.year}-01-01`;
  ...
  const metadata = {
    id:`official-${source.id}`,title:source.title,authority:"The National Archives / legislation.gov.uk",jurisdiction:source.jurisdiction,
    canonical_location:base,version,retrieved_at:retrievedAt,publication_date:madeDate,effective_date:retrievedAt.slice(0,10),
    expiry_date:source.expiry_date || null,licence:"Open Government Licence v3.0",reviewer:"official-source-import",
    approval_status:"approved",source_type:"secondary_legislation",authority_rank:1,
    oscola_citation:`${source.title}, ${citationPrefix} ${source.year}/${source.number}`,document_type:"law",
    document_status:documentStatus,snapshot_valid_from:snapshotValidFrom,unapplied_effects:unappliedEffects,
    jurisdiction_pair:source.pair,text_extraction:textExtraction,topics:source.topics,
    status_note:"Latest available legislation.gov.uk consolidated snapshot paired with the as-made source. Check commencement, amendments, transitional provisions and territorial extent before relying on a provision.",
  ...
  console.log(`${Math.round(original.bytes/1024)} KB ${original.format.toUpperCase()}, ${Math.round(text.length/1024)} KB text`);
}
