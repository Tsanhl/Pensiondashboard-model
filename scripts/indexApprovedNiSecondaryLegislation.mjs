...
const directory = resolve("approved-materials","index","ni-secondary-legislation");
...
if (requestedIds.size && metadataFiles.length !== requestedIds.size) throw new Error("One or more MATERIAL_IDS did not match downloaded Northern Ireland secondary-legislation metadata");
...
for (const filename of metadataFiles) {
  ...
  const result = await indexDocument("__public__",{
    ...
    metadata:{ reviewer:metadata.reviewer,approvalStatus:metadata.approval_status,sourceType:metadata.source_type,
      ...
      unappliedEffects:metadata.unapplied_effects,counterpartOf:metadata.counterpart_of,mappingStatus:metadata.mapping_status,
      textExtraction:metadata.text_extraction,
      seminars:metadata.seminars,topics:metadata.topics,statusNote:metadata.status_note,
