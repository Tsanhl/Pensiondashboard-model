...
const directory = resolve("approved-materials","index","case-law");
...
if (requestedIds.size && metadataFiles.length !== requestedIds.size) throw new Error("One or more MATERIAL_IDS did not match downloaded case-law metadata");
...
for (const filename of metadataFiles) {
  ...
  const result = await indexDocument("__public__",{
    ...
    metadata:{ reviewer:metadata.reviewer,approvalStatus:metadata.approval_status,sourceType:metadata.source_type,
      ...
      court:metadata.court,neutralCitation:metadata.neutral_citation,seminars:metadata.seminars,topics:metadata.topics,
