...
const compactRoot = path.resolve(
  process.env.WAVE2_FINAL_COMPACT_ROOT ||
    path.join(
      workspaceRoot,
...
const recommendationsPath = path.resolve(
  process.env.WAVE2_REVIEWED_RECOMMENDATIONS ||
    path.join(
      workspaceRoot,
      "training/evaluation-cycle-v2/23-cumulative-visible-qualification-v8-final-full-20260901/independent-agent-audits/wave-2-compact-substantive-retention-recommendations.v8-root23-provisional-v1.json",
    ),
);
const itemRegisterPath = path.join(
  compactRoot,
...
const compactMapPath = path.join(compactRoot, "compact-evidence-map.json");
...
const EXPECTED = {
  ...
  reviewed_recommendations_sha256:
    "df2ce42b4f2dec5f616f4fb9119aee3c6cc7f9ec3094bf1cec981cf4333521b4",
};
...
const bindings = {
  item_register_sha256: sha256(itemRegisterRecord.bytes),
  full_source_register_sha256: sha256(sourceRegisterRecord.bytes),
  compact_evidence_map_sha256: sha256(compactMapRecord.bytes),
  reviewed_recommendations_sha256: sha256(recommendationsRecord.bytes),
};
for (const [name, expected] of Object.entries(EXPECTED)) {
  if (bindings[name] !== expected) {
    throw new Error(
      `Immutable root-24 binding changed for ${name}: ${bindings[name]} != ${expected}.`,
    );
  }
}

const itemRegister = itemRegisterRecord.value;
const sourceRegister = sourceRegisterRecord.value;
...
const wave2TrainingIds = new Set(wave2Items.map((item) => item.training_id));

if (wave2Items.length !== 33 || wave2TrainingIds.size !== 33) {
  throw new Error(
    `Expected 33 unique Wave 2 items; found ${wave2Items.length}/${wave2TrainingIds.size}.`,
  );
}
if (
  recommendations.decisions?.length !== 127 ||
  recommendations.counts?.exact_span_decisions !== 127 ||
  recommendations.passed !== false ||
  recommendations.training_authorised !== false ||
  recommendations.release_authorised !== false ||
  recommendations.independent_legal_review !== false ||
  recommendations.unseen_accessed !== false
) {
  throw new Error(
    "Root-23 recommendations must contain exactly 127 fail-closed reviewed decisions.",
  );
}
if (
  compactMap.span_recommendation_bindings?.wave_2_sha256 !==
  bindings.reviewed_recommendations_sha256
) {
  throw new Error(
    "The compact map is not bound to the supplied Wave 2 recommendations.",
  );
}

const sourceById = new Map();
for (const item of itemRegister.items) {
  for (const source of item.retrieved_evidence || []) {
    ...
    if (existing && sha256(existing.text) !== sha256(source.text)) {
      throw new Error(
        `Parent source ${source.source_id} has inconsistent text occurrences.`,
      );
    }
    sourceById.set(source.source_id, source);
  }
}
for (const identity of sourceRegister.sources) {
  const parent = sourceById.get(identity.source_id);
  if (!parent || sha256(parent.text) !== identity.content_sha256) {
    throw new Error(
      `Canonical source ${identity.source_id} is absent or differs from its parent bytes.`,
    );
  }
}

const itemMappingByKey = new Map(
  wave2Items.flatMap((item) =>
...
const finalMappings = compactMap.proposition_mappings.filter((mapping) =>
  wave2TrainingIds.has(mapping.training_id),
);
...
const propositionCheckByKey = new Map(
  compactMap.proposition_checks
    .filter((check) => wave2TrainingIds.has(check.training_id))
    .map((check) => [mappingKey(check.training_id, check.proposition), check]),
);
...
const findings = [];
const decisions = [];
const recordFinding = (recommendation, code, detail) => {
  findings.push({
    mapping_index: recommendation?.mapping_index ?? null,
    training_id: recommendation?.training_id ?? null,
    proposition: recommendation?.proposition ?? null,
    code,
    detail,
  });
};
...
if (
  finalMappings.length !== 127 ||
  finalMappingByKey.size !== 127 ||
  itemMappingByKey.size !== 127 ||
  propositionCheckByKey.size !== 127
) {
  findings.push({
    mapping_index: null,
    training_id: null,
    proposition: null,
    code: "wave_2_mapping_cardinality_mismatch",
    detail: `Final/item/check mappings: ${finalMappings.length}/${itemMappingByKey.size}/${propositionCheckByKey.size}; expected 127 unique rows.`,
  });
}
if (
  ...
  compactMap.item_register_sha256 !== bindings.item_register_sha256 ||
  compactMap.full_source_register_sha256 !==
    bindings.full_source_register_sha256
) {
  findings.push({
    mapping_index: null,
    training_id: null,
    ...
    detail:
      "The compact map's scope flags or item/source register bindings do not satisfy the fail-closed development contract.",
  });
}
if (
...
for (const recommendation of recommendations.decisions) {
  ...
  if (!mapping || !itemMapping || !propositionCheck) {
    recordFinding(
      ...
      "The exact reviewed item/proposition is absent from the final map, item map, or proposition checks.",
    );
    continue;
  }

  if (sha256(mapping.proposition) !== recommendation.proposition_sha256) {
    recordFinding(
      recommendation,
      "proposition_hash_mismatch",
      "The proposition bytes changed after the source-scope review.",
    );
  }
  if (
  ...
  if (
    ...
    recordFinding(
      ...
      "The final compact mapping uses different parent sources from the substantively reviewed recommendation.",
    );
  }
  if (
  ...
  for (const excerptId of mapping.excerpt_ids || []) {
    ...
    if (
      ...
      excerpt.full_source_sha256 !== sha256(parent.text) ||
      ...
      recordFinding(
        recommendation,
        "excerpt_parent_or_hash_failure",
        `Excerpt ${excerptId} is not an exact, selected-parent substring with valid hashes.`,
      );
    }
  }
  const finalExcerptParentIds = finalExcerpts.map(
    (excerpt) => excerpt.parent_source_id,
  );
  if (
    !sameSet(finalExcerptParentIds, mapping.selected_parent_source_ids)
  ) {
    recordFinding(
      recommendation,
  ...
  for (const reviewedSpan of recommendation.recommended_spans) {
    ...
    if (
      ...
      recordFinding(
        ...
        `Reviewed span ${reviewedSpan.parent_source_id} [${reviewedSpan.start},${reviewedSpan.end}) no longer matches root-24 parent bytes.`,
      );
      continue;
    }
    if (
      ...
      recordFinding(
        recommendation,
        "reviewed_material_condition_not_retained",
        `No final excerpt covers the complete reviewed span ${reviewedSpan.parent_source_id} [${reviewedSpan.start},${reviewedSpan.end}).`,
      );
    }
  }
...
const uniqueDecisionKeys = new Set(
...
const passed =
  findings.length === 0 &&
  decisions.length === 127 &&
  uniqueDecisionKeys.size === 127;
const result = {
  version: "wave-2-compact-substantive-retention-audit-v1",
  ...
  counts: {
    ...
    reviewed_mappings: passed ? 127 : decisions.length,
    retained_complete: passed ? 127 : 0,
