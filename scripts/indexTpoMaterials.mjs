...
const directory = resolve("approved-materials","index","pensions-ombudsman");
...
for (const filename of (await readdir(directory)).filter((name)=>name.endsWith("-metadata.json")).sort()) {
  ...
  const result = await indexDocument("__public__",{
    ...
    metadata:{ reviewer:metadata.reviewer,approvalStatus:metadata.approval_status,sourceType:metadata.source_type,
      ...
      updateCycleDays:metadata.update_cycle_days,decisionReference:metadata.decision_reference || null,
      outcome:metadata.outcome || null,complaintTopic:metadata.complaint_topic || null,statusNote:metadata.status_note,
