...
const directory = resolve("approved-materials","index","legislation");
...
if (requestedIds.size && metadataFiles.length !== requestedIds.size) throw new Error("One or more MATERIAL_IDS did not match downloaded legislation metadata");
...
for (const filename of metadataFiles) {
  ...
  const result = await indexDocument("__public__", {
    ...
    metadata:{ reviewer:metadata.reviewer,approvalStatus:metadata.approval_status,sourceType:metadata.source_type,authorityRank:metadata.authority_rank,oscolaCitation:metadata.oscola_citation,retrievedAt:metadata.retrieved_at,snapshotValidFrom:metadata.snapshot_valid_from,documentStatus:metadata.document_status,unappliedEffects:metadata.unapplied_effects,statusNote:metadata.status_note,originalSource:metadata.files.original?.path || metadata.files.pdf?.path,currentXml:metadata.files.xml.path,updateCycleDays:30 }
  });
