...
const originals = resolve(root,"originals","eu-legislation");
const index = resolve(root,"index","eu-legislation");
await Promise.all([mkdir(originals,{ recursive:true }),mkdir(index,{ recursive:true })]);
const sources = JSON.parse(await readFile(resolve(root,"eu-legislation-sources.json"),"utf8"));
...
for (const source of sources) {
  ...
  const metadata = {
    id:`official-eu-law-${source.id}`,title:source.title,authority:"European Union / Publications Office of the European Union",
    jurisdiction:"European Union; UK effect depends on event date and the current assimilated/retained-EU-law framework",
    canonical_location:canonical,celex:source.celex,official_xhtml_url:htmlItem.item,official_pdf_url:pdfItem.item,
    version,retrieved_at:retrievedAt,publication_date:source.publication_date,effective_date:source.publication_date,expiry_date:null,
    licence:"European Commission reuse policy",licence_url:"https://commission.europa.eu/legal-notice_en#copyright-notice",
    reviewer:"official-source-import",approval_status:"approved",source_type:"eu_legislation",authority_rank:0.95,
    oscola_citation:source.title,document_type:"law",seminars:source.seminars,topics:source.topics,
    status_note:"Official EU legislative text. For a UK answer, determine the relevant event date and retrieve the active UK withdrawal, assimilated-law, implementing and amending provisions before treating an EU rule as applicable.",
  ...
  console.log(`${source.title}: ${Math.round(text.length/1024)} KB text`);
}
