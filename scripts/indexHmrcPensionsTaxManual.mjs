...
const directory = resolve("approved-materials","index","hmrc-pensions-tax-manual");
const requestedIds = new Set(String(process.env.MATERIAL_IDS || "").split(",").map((id)=>id.trim().toLowerCase()).filter(Boolean));
...
if (requestedIds.size && metadataFiles.length !== requestedIds.size) throw new Error("One or more MATERIAL_IDS did not match HMRC PTM metadata");
...
for (const filename of metadataFiles) {
  ...
  const result = await indexDocument("__public__",{
    ...
    metadata:{ reviewer:metadata.reviewer,approvalStatus:metadata.approval_status,sourceType:metadata.source_type,
      ...
      sourceUpdatedAt:metadata.source_updated_at,updateCycleDays:metadata.update_cycle_days,pageCount:metadata.page_count,
      topics:metadata.topics,statusNote:metadata.status_note }
  });
