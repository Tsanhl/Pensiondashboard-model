...
const SRC = join(ROOT, "Log/2026-09-02/live-round-52");
const OUT = join(ROOT, "Log/2026-09-02/live-round-52-final");
...
const r51 = existsSync(join(ROOT, "Log/2026-09-02/live-round-51-adjudicated/adjudicated-results.json"))
  ? JSON.parse(readFileSync(join(ROOT, "Log/2026-09-02/live-round-51-adjudicated/adjudicated-results.json"), "utf8"))
  : { items: [] };
const specById = Object.fromEntries((BANK.questions || []).map((item) => [item.id, item]));
const r51ById = Object.fromEntries((r51.items || []).map((item) => [item.caseId, item]));
...
if (!existsSync(join(OUT, "results.round52-immutable.json"))) {
  cpSync(join(SRC, "results.json"), join(OUT, "results.round52-immutable.json"));
}
...
writeFileSync(join(OUT, "results.json"), `${JSON.stringify({
  ...
  source: "Log/2026-09-02/live-round-52",
  overlay: Object.keys(overlays),
  complete: 50,
  total: 50,
  items: mergedItems
}, null, 2)}\n`);
writeFileSync(join(OUT, "adjudicated-results.json"), `${JSON.stringify({ generated_at: new Date().toISOString(), source: "Log/2026-09-02/live-round-52", overlay: Object.keys(overlays), counts, items }, null, 2)}\n`);
...
const comparison = items.map((row) => ({
  caseId: row.caseId,
  round51: r51ById[row.caseId]?.verdict || null,
  round52: row.verdict,
...
writeFileSync(join(OUT, "regression-comparison.json"), `${JSON.stringify({
  ...
  changed: comparison.filter((row) => row.round51 && row.round51 !== row.round52),
  items: comparison
}, null, 2)}\n`);
...
writeFileSync(join(OUT, "DEVELOPMENT-GATE-REPORT.md"), `# Live-50 development gate (round 52)

This is a **development regression** result. It is not qualification, sealed-unseen evaluation, or a live-release claim.
...
Document regression vs Round 51 PASS: ${documentRegression.join(", ") || "none"}
